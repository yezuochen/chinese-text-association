...
1. Tokenize each doc with `jieba.cut`; keep tokens with `len(token) ≥ 2`; drop stopwords.
...
6. Attach computed `domain_ratio_math` and `domain_經濟金融` to each concept node — **derived attributes**, not hand-coded.
...
## 4. L2 — OpenAI text-embedding-3-small K-NN File Graph (full corpus, paid, API)
...
### 4.2 Method: OpenAI text-embedding-3-small + FAISS K-NN

**Embedding model**: OpenAI `text-embedding-3-small` (1536-dim, ~US$0.02 for full corpus)
**Runtime**: API calls via `openai` SDK; batch size 100; 3000-char truncation per document
**Rate-limit handling**: Exponential backoff retry (1.5s → 3s → 5s → 8s → 10s max); 5 retries per batch
**Why not local BGE-M3**: RTX 3050 CUDA unavailable via `uv` (CPU wheel installed); BGE-M3 on CPU too slow
...
resp = client.embeddings.create(
    input=[truncate(t, 3000) for t in texts],
...
1. L2-normalize all ~7,378 document vectors (cosine similarity via IP search).
2. FAISS `IndexFlatIP` (CPU).
3. K=15 (+ 1 self, excluded to remove self-loops).
4. Cosine similarity range: 0.2947 – 1.0000.
...
2. **Concept nodes**:
   ...
   - L1 PMI nodes not in L3: `type='concept'`, `source='pmi'`, `domain_ratio_math`, `domain_經濟金融`
...
| L2 embeddings | `analysis/embeddings.parquet` has 9,943 rows; each `embedding` is length 1536 |
...
| Cross-domain bridge | 期望值's L3 source docs span both folders; one of its L3 communities has mixed `domain_ratio_math` |
...
|-------------|--------|----------------------|
| **Bloomberg / Reuters** | Structured tickers + 中文新聞 | Append as new domain; entity extraction targets instruments (AAPL, TSMC) and macro events; relations ground in price/action semantics |
...
5. **LLM cost management**: Live financial data at scale (e.g., 10,000 Bloomberg articles/day) would make L3 GraphRAG prohibitively expensive. Consider a hybrid: L1 PMI on daily batch, L2 text-embedding-3-small K-NN on daily batch, L3 GraphRAG only on high-signal events (earnings surprises, Fed decisions) identified via L1/L2 anomaly scoring.

### 11.4 What stays the same

- **L1 (PMI)**: Same algorithmic core; data schema needs `timestamp` column and `time-windowed` PMI variant; stopword set expanded with financial-domain terms (e.g., "公布", "本報告", "根據").
- **L2 (text-embedding-3-small)**: Model unchanged; re-embed new documents via the same OpenAI API.
